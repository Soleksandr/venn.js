...
export function VennDiagram({
  ...
  function chart(selection) {
    ...
    data = data.sort(
      (a, b) => a.sets.length - b.sets.length || b.size - a.size
    );
    ...
    // --- start: logic to move circle to the top in case 3 circles view without intersections
    let intersectionsData = [];
    data.forEach((item) => {
      const cs = item.sets.map((set) => circles[set]);

      const intersection = {};
      intersectionArea(cs, intersection);

      intersectionsData.push(intersection);
    });

    const threeCirclesWithoutIntersections =
      intersectionsData.length === 3 &&
      intersectionsData.every(
        (i) => !i.innerPoints.length && !i.intersectionPoints.length
      );

    if (threeCirclesWithoutIntersections) {
      const circlesArr = Object.values(circles);
      const maxY = Math.max(...circlesArr.map((c) => c.y));
      const maxR = Math.max(...circlesArr.map((c) => c.radius));
      const bottomCircle = circlesArr.find((c) => c.y === maxY);
      const topCircles = circlesArr.filter(
        (c) => c.setId !== bottomCircle.setId
      );
      bottomCircle.y =
        bottomCircle.y -
        Math.sqrt(Math.pow(maxR + bottomCircle.radius, 2) - Math.pow(maxR, 2)) -
        2.5;
      bottomCircle.x = bottomCircle.x + maxR + 1.25;
      topCircles.map((c) => {
        c.y = maxY;
        return c;
      });
